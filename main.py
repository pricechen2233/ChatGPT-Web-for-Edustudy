...
import requests
from flask import Flask, render_template, request, session, send_file, make_response, jsonify, url_for, redirect
...
API_KEY = os.getenv("API_KEY", default=API_KEY)  # 如果环境变量中设置了OPENAI_API_KEY，则使用环境变量中的OPENAI_API_KEY
...
project_info = "你好，我是GPT，有什么问题可以尽管问我！"
...
def login():
    ...
    if success:
        ...
        session['user_id'] = user_id  # 设置会话数据
        session['password'] = password
        # 设置完会话数据后，进行重定向到 'profile' 路由
        return jsonify({'redirect': url_for('gpt'),
                        'chats': load_chats()['data'],
                        'messages_of_chats': load_messages()['data']})
        # return jsonify({'redirect': url_for('gpt'),
        #                 'chats': load_chats()['data'],
        #                 'messages_of_chats': load_messages()['data']})
    else:
...
def gpt():
    ...
    :return: 聊天页
    """
    num_session = check_session(session)
    print("gpt page of session:",num_session)
    if 'user_id' in session:
        # 获取用户ID
        user_id = session['user_id']
        return render_template('gpt.html', user_id=user_id)
    else:
        # 未登录用户重定向到登录页面
        return redirect('/')
...
def load_messages():
    ...
    """
    num_session = check_session(session)
    print("loadhistory of session:",num_session)
    success, message = auth(request.headers, session)
    ...
    if session.get('user_id') is None:
        ...
        if session.get('password') is None:
            return {"code": code, "data": None, "message": "请返回初始页面并输入密码"}
        user_info = get_user_info(session.get('user_id')+','+session.get('password'))
...
def auth(request_head, session):
    ...
    if user_info is not None:
        ...
        if session.get('user_id') is not None:
            del session['user_id']
        return False, "用户不存在，请在设置中填写正确的用户id和密码"
...
def load_chats():
    ...
    """
    num_session = check_session(session)
    print("loadchats of session",num_session)
    success, message = auth(request.headers, session)

    if not check_user_bind(session) or not success:
        print(check_user_bind(session)," loadChats cheak")
        chats = []
    else:
        if session.get('password') is None:
            return {"code": 200, "data": None, "message": "请返回初始页面并输入密码"}
        user_info = get_user_info(session.get('user_id') + ',' + session.get('password'))
        chats = []
        chat_ids = []
        # 如果置顶聊天列表不为空
...
def return_message():
    ...
    url_redirect = {"url_redirect": "/", "user_id": None, "password": None}
    if send_message == "帮助":
        return "### 帮助\n" \
               "1. 输入`newcreate:xxx:xxx`创建新的用户id和密码\n " \
    ...
    if session.get('user_id') is None:  # 如果当前session未绑定用户
        ...
        if send_message.startswith("id:"):
            ...
            password = send_message.split(":")[2].strip()
            user_info = get_user_info(user_id+','+password)
            if user_info is None:
...
def check_load_pickle():
    ...
    if os.path.exists(os.path.join(DATA_DIR, USER_DICT_FILE)):
        with open(os.path.join(DATA_DIR, USER_DICT_FILE), "rb") as pickle_file:
            all_user_dict = cPickle.load(pickle_file)
        ...
        with open(os.path.join(DATA_DIR, USER_DICT_FILE), "wb") as pickle_file:
            cPickle.dump(all_user_dict, pickle_file)
